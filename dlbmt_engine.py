...
class Controller:
    ...
    id: str
    capacity_cpu: float = 2000.0    
    capacity_mem: float = 4096.0    
    capacity_bw: float = 1000.0     
...
class DLBMTEngine:
    ...
    def compute_switch_resource_usage(self, switch: Switch, controller: Controller) -> float:
        """
        £_ji = a·(LoadCPU/CPU) + b·(LoadMem/Mem) + c·(LoadBw/Bw)
    ...
    def compute_switch_resource_on_target(self, switch: Switch, target_ctrl: Controller) -> float:
        """
        £_kij = a·(LoadCPU/CPU_k) + b·(LoadMem/Mem_k) + c·(LoadBw/Bw_k)
    ...
    def compute_controller_load(self, controller_id: str) -> float:
        """
        £_j = Σ £_ji for all switches in controller's domain
        LR_cj = £_j * 100
    ...
    def determine_level(self, load: float) -> ControllerLevel:
        """
        Determine controller level based on load.
    ...
    def _execute_migration(self, switch: Switch, source_ctrl: Controller, target_ctrl: Controller, data: dict) -> MigrationRecord:

        dc_before = data.get("dc_before", 0)

        # Capture global imbalance BEFORE migration
        global_imb_before = self._global_imbalance()
        loads_before = {c.id: round(c.load_percentage, 2) for c in self.controllers.values() if c.active}
        ...
        loads_after = {c.id: round(c.load_percentage, 2) for c in self.controllers.values() if c.active}
    ...
    def run_load_balancing(self) -> Optional[MigrationRecord]:
        """
        Complete load balancing algorithm.
